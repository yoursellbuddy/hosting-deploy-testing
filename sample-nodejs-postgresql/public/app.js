/* ==========================================================================
   Node.js & PostgreSQL Testing App - Frontend Client Logic
   ========================================================================== */
...
function initTheme() {
    ...
    const savedTheme = localStorage.getItem('nodepostgres_theme') || 'dark';
    ...
    themeToggleBtn.addEventListener('click', () => {
        ...
        localStorage.setItem('nodepostgres_theme', newTheme);
...
async function recheckHealth() {
    try {
        ...
        if (data.database.connected) {
            ...
            dbStatusBadge.innerHTML = '<span class="pulse-dot green"></span> PostgreSQL: Connected';
            ...
            dbStatusBadge.innerHTML = '<span class="pulse-dot red"></span> PostgreSQL: Offline';
            ...
            appendLog(`[PostgreSQL ERROR] ${data.database.error || 'Database unreachable'}`, 'error');
        }
...
// 3. PostgreSQL Benchmark & Query Latency Check
async function runDbBenchmark() {
    try {
        ...
        if (data.connected) {
            ...
            // Extract short PG version (e.g. PostgreSQL 16.2)
            const shortVer = data.pgVersion ? data.pgVersion.split(' ')[0] + ' ' + data.pgVersion.split(' ')[1] : 'PostgreSQL';
            document.getElementById('lblDbVersion').textContent = shortVer;

            appendLog(`[PostgreSQL PING] Query Ping: ${data.pingMs}ms | Version: ${shortVer} | Tables: ${data.tableCount} | Items: ${data.recordsCount}`, 'success');
        } else {
            document.getElementById('lblDbPing').textContent = '-- ms';
            document.getElementById('lblDbVersion').textContent = '--';
        }
    } catch (e) {
        appendLog(`[PostgreSQL] Benchmark request error: ${e.message}`, 'warning');
    }
}
...
async function loadItems() {
    ...
    try {
        ...
        if (data.items.length === 0) {
            tbody.innerHTML = `<tr><td colspan="6" class="text-center text-muted">No records in \`test_items\` table. Add your first record above!</td></tr>`;
            return;
        }
        ...
        appendLog(`[CRUD SELECT] Retrieved ${data.items.length} records from \`test_items\``, 'info');
    } catch (err) {
...
async function runPendingMigrations() {
    appendLog('[MIGRATOR] Requesting pending PostgreSQL database migrations execution...', 'info');
    try {
...
function initForm() {
    ...
    form.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            if (res.ok && data.success) {
                appendLog(`[CRUD INSERT] Inserted Record #${data.item.id} ("${data.item.title}") into PostgreSQL`, 'success');
            ...
            btnSubmit.innerHTML = '➕ Insert into PG DB';
        }
    });
}
...
async function deleteItem(id) {
    ...
    try {
        ...
        if (res.ok && data.success) {
            appendLog(`[CRUD DELETE] Deleted Record #${id} from PostgreSQL`, 'warning');
            loadItems();
        } else {
