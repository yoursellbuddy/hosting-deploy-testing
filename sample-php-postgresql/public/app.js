/* ==========================================================================
   Laravel PHP & PostgreSQL Testing App - Frontend Client Logic
   ========================================================================== */
...
function initTheme() {
    ...
    const savedTheme = localStorage.getItem('laravel_pg_theme') || 'dark';
    ...
    themeToggleBtn.addEventListener('click', () => {
        ...
        localStorage.setItem('laravel_pg_theme', newTheme);
...
async function recheckHealth() {
    try {
        ...
        // PHP status
        document.getElementById('lblPhpVersion').textContent = data.phpVersion || 'PHP 8.2+';
        document.getElementById('lblPhpEnv').textContent = `Environment: ${data.environment}`;
        ...
        appendLog(`[HTTP ERROR] Failed to connect to PHP backend: ${err.message}`, 'error');
    }
}
...
async function runDbBenchmark() {
    try {
        ...
        if (data.connected) {
            ...
            // Short PG Version string
            const shortVer = data.pgVersion ? data.pgVersion.split(' ')[0] + ' ' + data.pgVersion.split(' ')[1] : 'PostgreSQL 16';
            document.getElementById('lblDbVersion').textContent = shortVer;

            appendLog(`[PDO PING] Query Ping: ${data.pingMs}ms | PG Ver: ${shortVer} | Tables: ${data.tableCount} | Items: ${data.recordsCount}`, 'success');
        } else {
            document.getElementById('lblDbPing').textContent = '-- ms';
            document.getElementById('lblDbVersion').textContent = '--';
        }
    } catch (e) {
        appendLog(`[PDO] Benchmark request error: ${e.message}`, 'warning');
    }
}
