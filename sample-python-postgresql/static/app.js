/* ==========================================================================
   Python Flask & PostgreSQL Testing App - Frontend Client Logic
   ========================================================================== */
...
function initTheme() {
    ...
    const savedTheme = localStorage.getItem('python_pg_theme') || 'dark';
    ...
    themeToggleBtn.addEventListener('click', () => {
        ...
        localStorage.setItem('python_pg_theme', newTheme);
...
// 3. psycopg2 Benchmark & Query Latency Check
async function runDbBenchmark() {
    try {
        ...
        if (data.connected) {
            ...
            appendLog(`[psycopg2 PING] Query Ping: ${data.pingMs}ms | PG Ver: ${shortVer} | Tables: ${data.tableCount} | Items: ${data.recordsCount}`, 'success');
        } else {
            document.getElementById('lblDbPing').textContent = '-- ms';
            document.getElementById('lblDbVersion').textContent = '--';
        }
    } catch (e) {
        appendLog(`[psycopg2] Benchmark request error: ${e.message}`, 'warning');
    }
}
